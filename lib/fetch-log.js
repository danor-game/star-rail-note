...
const fetchLog = async (uid) => {
	...
	for(const typeGacha in M.gacha.name$Type) {
		const nameTypeGacha = M.gacha.name$Type[typeGacha];
		...
		for(let page = 0; ; page++) {
			...
			if(logsParsed.find(logParsed => logParsed.id == idEndScope)) { break; }
				...
				...logsPage
					.filter(logRaw => {
						...
						if(
							...
							logParsed.time == Day(`${logRaw.time} ${hourToTimezone(logRaw.time)}`, 'YYYY-MM-DD HH:mm:ss Z').unix()
						) {
