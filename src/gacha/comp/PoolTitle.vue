...
const $textRangeTimePool = computed(() =>
	$pool.value.timeDead
		? $pool.value.timeBorn
			? `${Day($pool.value.timeBorn, 'X').format('MM月DD HH时mm分')} ~ ${Day($pool.value.timeDead, 'X').format('MM月DD HH时mm分')}`
			: `维护后 ~ ${Day($pool.value.timeDead, 'X').format('MM月DD HH时mm分')}`
		: false
);
...
p-pool-title
	...
	background-color: color-mix(in srgb, var(--cBack) 97%, var(--cText))

	p-header
		@apply inblock w-12 h-full leading-[calc(var(--spc)*12)] text-center
		img
	...
	p-name-box
		@apply inblock ml-2
		p-boost
			@apply inline text-xs font-normal elli p-0.5 mr-2
			&[_rarity-4]
				@apply text-[var(--cRarity4)] border-b border-[var(--cRarity4)]
			&[_rarity-5]
